export interface ServicesData {
  title: string;
  description: string;
  icon: string;
  color: string;
}

export const colors: string[] = ['all', 'yellow', 'green-full', 'green-stroke', 'gray']
export const occupations: string[] = ['All', 'Instructional Design', 'Elearning Development', 'Mobile Learning', 'Instructor-led Training']

export const serviceData: ServicesData[] = [
  {
    title: "Instructional Design",
    description:
      "We provide full-service instructional design consulting. From standalone lessons to the series of courses, no job is too large or too small for us.",
    icon: "icons/instr-design-white.svg",
    color: "yellow",
  },
  {
    title: "Elearning Development",
    description:
      "The eLearning products we create feature deep learner engagement and ensure high knowledge transfer.",
    icon: "icons/elearn-dev-white.svg",
    color: "yellow",
  },
  {
    title: "Mobile Learning",
    description:
      "Let us help you take your learning process anywhere your business may take you. Any place, any time, any device.",
    icon: "icons/mobile-learn-white.svg",
    color: "yellow",
  },
  {
    title: "Instructor-led Training",
    description:
      "We are experts at developing a classroom curriculum that meets your business needs and exceeds even your highest expectations.",
    icon: "icons/instr-led-training-white.svg",
    color: "yellow",
  },
  {
    title: "Instructional Design",
    description:
      "We provide full-service instructional design consulting. From standalone lessons to the series of courses, no job is too large or too small for us.",
    icon: "icons/instr-design-white.svg",
    color: "green-full",
  },
  {
    title: "Elearning Development",
    description:
      "The eLearning products we create feature deep learner engagement and ensure high knowledge transfer.",
    icon: "icons/elearn-dev-white.svg",
    color: "green-full",
  },
  {
    title: "Mobile Learning",
    description:
      "Let us help you take your learning process anywhere your business may take you. Any place, any time, any device.",
    icon: "icons/mobile-learn-white.svg",
    color: "green-full",
  },
  {
    title: "Instructor-led Training",
    description:
      "We are experts at developing a classroom curriculum that meets your business needs and exceeds even your highest expectations.",
    icon: "icons/instr-led-training-white.svg",
    color: "green-full",
  },
  {
    title: "Instructional Design",
    description:
      "We provide full-service instructional design consulting. From standalone lessons to the series of courses, no job is too large or too small for us.",
    icon: "icons/instr-design-green.svg",
    color: "green-stroke",
  },
  {
    title: "Elearning Development",
    description:
      "The eLearning products we create feature deep learner engagement and ensure high knowledge transfer.",
    icon: "icons/elearn-dev-green.svg",
    color: "green-stroke",
  },
  {
    title: "Mobile Learning",
    description:
      "Let us help you take your learning process anywhere your business may take you. Any place, any time, any device.",
    icon: "icons/mobile-learn-green.svg",
    color: "green-stroke",
  },
  {
    title: "Instructor-led Training",
    description:
      "We are experts at developing a classroom curriculum that meets your business needs and exceeds even your highest expectations.",
    icon: "icons/instr-led-training-green.svg",
    color: "green-stroke",
  },
  {
    title: "Instructional Design",
    description:
      "We provide full-service instructional design consulting. From standalone lessons to the series of courses, no job is too large or too small for us.",
    icon: "icons/instr-design-gray.svg",
    color: "gray",
  },
  {
    title: "Elearning Development",
    description:
      "The eLearning products we create feature deep learner engagement and ensure high knowledge transfer.",
    icon: "icons/elearn-dev-gray.svg",
    color: "gray",
  },
  {
    title: "Mobile Learning",
    description:
      "Let us help you take your learning process anywhere your business may take you. Any place, any time, any device.",
    icon: "icons/mobile-learn-gray.svg",
    color: "gray",
  },
  {
    title: "Instructor-led Training",
    description:
      "We are experts at developing a classroom curriculum that meets your business needs and exceeds even your highest expectations.",
    icon: "icons/instr-led-training-gray.svg",
    color: "gray",
  },
];
